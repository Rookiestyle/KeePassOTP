	...
	public static class TFASites
	{
		...
		private static string CreatePattern(string url)
		{
			...
			if (m.Groups.Count == 5) //4 matches + overall pattern
			{
				...
				if (_dCCSLD.ContainsKey(m.Groups[4].Value))
				{
					foreach (var s in _dCCSLD[m.Groups[4].Value])
					{
						bCCSLD = m.Groups[0].Value.EndsWith("." + s);
						if (bCCSLD)
						{
							tld = s;// (m.Groups[3].Value + "." + m.Groups[4].Value).Replace(".", @"\.");
							string sHelp = m.Groups[0].Value.Substring(0, m.Groups[0].Value.Length - s.Length);
							string[] aParts = sHelp.Split(new char[] { '.', '/' }, StringSplitOptions.RemoveEmptyEntries);
							bCCSLD = aParts.Length > 0;
							if (bCCSLD)
							{
								domain = aParts[aParts.Length - 1];
								tld = tld.Replace(".", @"\.");
			...
			else
			{
				...
				//special handling for domains like com.au or co.uk
				if (i > 1 && (parts[i - 1] == "com" || parts[i - 1] == "co"))
					domain = parts[i - 2] + @"\." + domain;
		...
		private static Dictionary<string, List<string>> _dCCSLD = new Dictionary<string, List<string>>();
		static TFASites()
        {
            //Prepare list of ccSLD (country code Second Level Domains) like co.uk, com.au, gov.ae, ...
            //List is taken from  https://publicsuffix.org/list/public_suffix_list.dat

			List<string> lCCSLD = sCCSLD.Split('\n').ToList();
			for (int i = 0; i < lCCSLD.Count; i++)
			{
				var s = lCCSLD[i].Trim().ToLowerInvariant().TrimStart(new char[] { '*', '.' });
				if (s.StartsWith("/") || string.IsNullOrEmpty(s)) continue;
				List<string> lParts = s.Split('.').ToList();
				if (lParts.Count == 1)
					continue;
				if (!_dCCSLD.ContainsKey(lParts[lParts.Count - 1])) _dCCSLD[lParts[lParts.Count - 1]] = new List<string>();
				if (!_dCCSLD[lParts[lParts.Count - 1]].Contains(s)) _dCCSLD[lParts[lParts.Count - 1]].Add(s);
			}
			foreach (KeyValuePair<string, List<string>> kvp in _dCCSLD)
			{
				kvp.Value.Sort((x, y) => x.Length == y.Length ? string.Compare(y, x) : y.Length.CompareTo(x.Length));
			}
		}

		#region Raw data ccSLD
		private const string sCCSLD = @"// This Source Code Form is subject to the terms of the Mozilla Public
